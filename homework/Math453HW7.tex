...
Let $S$ be the set of all $2\times 2$ matrices with real entries where all four entries are the same:
\[ 
S=\{\begin{pmatrix}x & x\\ x & x\end{pmatrix}\mid x\in\mathbb{R}\}
\] 
\begin{parts}
\part Is $S$ a ring under the usual matrix addition and multiplication? 
\part If so, is $S$ commutative? 
\part Does $S$ have an identity? 
\part Does $S$ have any zero divisors?
\end{parts}
